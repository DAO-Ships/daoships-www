...
The salt packing also differs by factory: `QuaiVaultFactory` uses
`keccak256(abi.encodePacked(address, bytes32))`, `DAOShipLauncher` uses
`keccak256(abi.encodePacked(address, uint256))`. And the vault is a `QuaiVaultProxy`, **not** an
