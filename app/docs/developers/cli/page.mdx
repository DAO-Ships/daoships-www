...
`@daoships/cli` provides **88 one-shot commands** and an interactive terminal workspace using the same command registry. The reviewed release is **0.1.0-alpha.3**, using SDK **0.1.0-alpha.4**. It supports Orchard and mainnet, all eight navigators and every function in the SDK's 17 contract interfaces.
...
For a reproducible install, use `@daoships/cli@0.1.0-alpha.3`. `ds` is an alias for `daoships`. Releases are published from matching GitHub version tags with npm provenance.
