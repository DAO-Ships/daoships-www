...
Launching a DAO Ships ship means deploying a complete on-chain organization — governance, two tokens, and a secured treasury — in a single transaction. The app guides you through it with a **7-step wizard** at [app.daoships.org/launch](https://app.daoships.org/launch). This guide walks each step, explains what every field means, and offers sane defaults so you can launch with confidence.
...
Before you start, connect a Quai wallet (such as Pelagus) in the app and make sure it holds a little QUAI for gas.

<Callout type="tip" title="Want to rehearse first?">
The same wizard runs against the Orchard testnet at [testnet.daoships.org/launch](https://testnet.daoships.org/launch), where gas is paid in free testnet QUAI from the [Quai faucet](https://faucet.quai.network). Launching there is a risk-free way to walk the whole flow before committing real funds.
</Callout>
